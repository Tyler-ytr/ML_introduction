...
def sigmod(Xi, beta):  # y=\frac{1}{1+e^{-\beta^{T}x}},返回的是一个浮点数,仅仅用于训练的时候用
    ...
    if (params > 300):
        params = 312
    # print(params)
    y = (1.0 / float(1.0 + math.exp(params)))
    return y


def sigmod_output_matrix(X, beta):  # 输入矩阵返回矩阵(数组)
    params =list(-np.dot(X, beta.T))
    #print(params)
    for i in range(len(params)):
        if (params[i] > 300):
            params[i] = 312
    r = np.zeros(len(params))  # 返回一个np数组
    # r = np.zeros(params.shape[0])  # 返回一个np数组
    for i in range(len(r)):
        r[i] = (1.0 / (1.0 + math.exp(params[i])))
    return r
...
class LogisticClassfier(object):  # 逻辑回归二分类器
    ...
    def fit(self, X, Y):  # X是16个标签构成的矩阵,Y是用于二分类class矩阵，值为0或1
        ...
        count = 1  # 调试用计数
        just_for_test=128000*9
        # 随机梯度下降法
        while True:
            if count>just_for_test:
            ...
            if count >self.loop+1:
                temp_sum_array = []
                temp_sum_array.clear()
                # for i in range(self.m):
                #     temp_sum += np.array(
                #         np.dot(float((sigmod(X_hat[i], self.beta) - Y[i])), X_hat[i]))
                #  尝试优化
                for i in range(self.m):
...
def score(my_result, target_result, alpha, loop, accuracy):
    ...
    dataframe.to_csv("resultr5.csv", index=False, sep=',')
    print(my_result)
    print(target_result)
